...
data "oci_core_vnic_attachments" "demotfBastionServer_VNIC1_attach" {
  availability_domain = var.availablity_domain_name == "" ? lookup(data.oci_identity_availability_domains.ADs.availability_domains[0], "name") : var.availablity_domain_name
  compartment_id      = oci_identity_compartment.DemoTF-Compartment.id
  instance_id         = oci_core_instance.demotfBastionServer.id
}

# Bastion Compute VNIC DataSource
data "oci_core_vnic" "demotfBastionServer_VNIC1" {
  vnic_id = data.oci_core_vnic_attachments.demotfBastionServer_VNIC1_attach.vnic_attachments.0.vnic_id
}
