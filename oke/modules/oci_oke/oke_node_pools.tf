...
resource "oci_containerengine_node_pool" "oke_node_pool" {
  ...
  dynamic "node_source_details" {
    ...
    content {
      ...
      source_type = data.oci_containerengine_node_pool_option.oke_node_pool_option.sources[0].source_type
    }
  }
  ...
  node_config_details {
    ...
    dynamic "node_pool_pod_network_option_details" {
        ...
        content {
          ...
          pod_subnet_ids    = var.use_existing_vcn ? [var.pods_subnet_id] :  [oci_core_subnet.oke_nodepool_subnet[0].id]
        }
    }
  }
...
resource "oci_containerengine_node_pool" "oke_node_pool" {
  ...
  # oke specific images
  dynamic "node_source_details" {
    for_each = var.node_pool_image_type == "oke" && local.oke_specific_image_id != null ? [1] : []
    content {
      boot_volume_size_in_gbs = var.node_pool_boot_volume_size_in_gbs
      image_id               = local.oke_specific_image_id
      source_type            = data.oci_containerengine_node_pool_option.oke_node_pool_option.sources[0].source_type
  ...
  # oci platform images
  dynamic "node_source_details" {
    for_each = var.node_pool_image_type == "platform" && local.oci_platform_image_id != null ? [1] : []
    content {
      boot_volume_size_in_gbs = var.node_pool_boot_volume_size_in_gbs
      image_id                = local.oci_platform_image_id
      source_type             = data.oci_containerengine_node_pool_option.oke_node_pool_option.sources[0].source_type
