...
resource "oci_core_security_list" "SQLNetSecurityList" {
  compartment_id = oci_identity_compartment.Terraform-Demo.id
...
resource "oci_core_subnet" "DBSubnet" {
  ...
  route_table_id             = oci_core_route_table.RouteTableViaNAT.id
  dhcp_options_id            = oci_core_dhcp_options.DhcpOptions1.id
  security_list_ids          = [oci_core_security_list.SSHSecurityList.id, oci_core_security_list.SQLNetSecurityList.id]
  prohibit_public_ip_on_vnic = true
}
