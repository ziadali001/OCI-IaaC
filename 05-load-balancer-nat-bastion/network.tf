...
resource "oci_core_security_list" "demotfWebSecurityList" {
  compartment_id = oci_identity_compartment.DemoTF-Compartment.id
  display_name   = "demotfWebSecurityList"
  vcn_id         = oci_core_virtual_network.demotfVCN.id
...
resource "oci_core_security_list" "demotfSSHSecurityList" {
  compartment_id = oci_identity_compartment.DemoTF-Compartment.id
  display_name   = "demotfSSHSecurityList"
  vcn_id         = oci_core_virtual_network.demotfVCN.id
...
resource "oci_core_subnet" "demotfWebSubnet" {
  ...
  security_list_ids          = [oci_core_security_list.demotfWebSecurityList.id, oci_core_security_list.demotfSSHSecurityList.id]
...
resource "oci_core_subnet" "demotfLBSubnet" {
  ...
  dhcp_options_id   = oci_core_dhcp_options.demotfDhcpOptions1.id
  security_list_ids = [oci_core_security_list.demotfWebSecurityList.id]
}
...
resource "oci_core_subnet" "demotfBastionSubnet" {
  ...
  route_table_id    = oci_core_route_table.demotfRouteTableViaIGW.id
  dhcp_options_id   = oci_core_dhcp_options.demotfDhcpOptions1.id
  security_list_ids = [oci_core_security_list.demotfSSHSecurityList.id]
}
