...
resource "null_resource" "demotfWebserver1HTTPD" {
  depends_on = [oci_core_instance.demotfWebserver1]
  provisioner "remote-exec" {
    connection {
      type        = "ssh"
      user        = "opc"
      host        = data.oci_core_vnic.demotfWebserver1_VNIC1.public_ip_address
...
resource "null_resource" "demotfWebserver2HTTPD" {
  depends_on = [oci_core_instance.demotfWebServer2]
  provisioner "remote-exec" {
    connection {
      type        = "ssh"
      user        = "opc"
      host        = data.oci_core_vnic.demotfWebserver2_VNIC1.public_ip_address
      private_key = tls_private_key.public_private_key_pair.private_key_pem
      script_path = "/home/opc/myssh.sh"
      agent       = false
      timeout     = "10m"
